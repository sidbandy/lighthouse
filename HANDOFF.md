...
- **`docs/KNOWN_GAPS.md` is the parking lot, and keeping it current is part of the job.** When you find a narrow edge case, a suspicious number, or a half-wired capability that is *not* blocking what you are building — **write it there instead of fixing it**. Chasing every small thing mid-feature is how a session ends with three half-finished modules; the operator asked for it to work this way explicitly.
  - Each entry must state **the problem, why it happens, and how to fix it**, with a severity of `misleading` / `incomplete` / `cosmetic`. An entry that only says "X is weird" costs the next session the same investigation that produced it.
  - **Anything that breaks a feature is not a gap — fix it immediately.** The parking lot is for things that do not affect the app working. A defect that records wrong data or produces a wrong number gets fixed on the spot, however small it looks. This was learned the hard way: the board originally stamped every stage with "now", which would have silently corrupted every wait-time figure the moment the operator back-filled a real search, and it was filed as a gap instead of a bug.
